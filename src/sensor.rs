...
impl SensorModel for SimpleLineSensor {
    fn sample(&mut self, track: &dyn TrackModel, pose: Pose2, t_us: u64) -> SensorOutput {
        ...
        let max_adc = (1u32.checked_shl(self.cfg.adc_bits).unwrap_or(0).saturating_sub(1)) as f64;
        let spacing = if count > 1 { self.cfg.width_m / (count as f64 - 1.0) } else { 0.0 };
        ...
        for i in 0..count {
            ...
            let reflectance = track.reflectance_at(world);
            let white_level = clamp01((reflectance - line) / denom);
            let line_signal = 1.0 - white_level;
            let value = (white_level * max_adc).round() as u32;
            adc.push(value);
