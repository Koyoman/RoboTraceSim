...
impl CsvLogger {
    pub fn create(path: &Path, sensor_count: usize) -> std::io::Result<Self> {
        ...
        let mut logger = Self { writer: BufWriter::new(file), sensor_count };
        logger.write_header()?;
        Ok(logger)
    }
    ...
    pub fn write_sample(&mut self, s: &TelemetrySample) -> std::io::Result<()> {
        write!(
            self.writer,
            "{},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{},{:.9},{:.6},{:.6},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9},{:.9}",
            s.t_us,
